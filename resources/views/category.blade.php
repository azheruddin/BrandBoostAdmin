...
<tbody>
    ...
    @endif
            ...
            <td>
    <form action="{{ route('update-cat-status', $category->id) }}" method="POST">
        @csrf
        <input type="hidden" name="id" value="{{ $category->id }}"> <!-- Add the category ID -->
        <input type="hidden" name="active" value="{{ $category->active == 1 ? 0 : 1 }}">
        
        <button type="submit" class="btn {{ $category->active == 1 ? 'btn-success' : 'btn-danger' }}">
            {{ $category->active == 1 ? 'Active' : 'Inactive' }} <!-- Update button text -->
        </button>
        <br>
        <small class="status-text text-muted">
            {{ $category->active == 1 ? 'Click to deactivate' : 'Click to activate' }}
        </small>
    </form>
</td>
